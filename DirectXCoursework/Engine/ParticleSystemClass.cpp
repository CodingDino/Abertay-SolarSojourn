...
// |						   Default Constructor								|
// |----------------------------------------------------------------------------|
ParticleSystemClass::ParticleSystemClass() : x(0), y(0), z(0)
{
	m_Texture = 0;
	m_particleList = 0;
	m_vertices = 0;
...
void ParticleSystemClass::EmitParticles(float frameTime)
{
	...
	if((emitParticle == true) && (m_currentParticleCount < (m_maxParticles - 1)))
	{
		...
		positionX = x + (((float)rand()-(float)rand())/RAND_MAX) * m_particleDeviationX;
...
void ParticleSystemClass::KillParticles()
{
	...
	for(i=0; i<m_maxParticles; i++)
	{
		// calculate distance between source and particle
		float distance_squared = pow((m_particleList[i].positionX-x),2)+pow((m_particleList[i].positionY-y),2)+pow((m_particleList[i].positionZ-z),2);

		if((m_particleList[i].active == true) && (distance_squared > 100.0f))
		{
			m_particleList[i].active = false;
			m_currentParticleCount--;
...
// |						       RenderBuffers								|
// |----------------------------------------------------------------------------|
void ParticleSystemClass::SetPosition(float new_x, float new_y, float new_z)
{
	x = new_x;
	y = new_y;
	z = new_z;
